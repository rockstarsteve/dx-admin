...
const user = {
    ...
    state: {
        token: cookieTokenUtil.getToken(),
        name: '',
        avatar: '',
        roles: [],
        permissions: []
    ...
    actions: {
        ...
        Login({commit}, userInfo) {
            ...
            return new Promise((resolve, reject) => {
                loginApi.login(username, password, code, uuid).then(res => {
                    console.info("登录后的结果是res : " + res.data)
                    cookieTokenUtil.setToken(res.data)
        ...
        GetInfo({commit, state}) {
            return new Promise((resolve, reject) => {
                loginApi.getInfo(state.token).then(res => {

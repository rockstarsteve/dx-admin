...
public class CustomerExportUtil {
     ...
     * @return
     */
    public static XSSFWorkbook getXSSFWorkbook(List<UserExam> listresult, String sheetName) {
...
//        String[] title = {"身份证", "名称"};
        ...
        UserExam userExam = listresult.get(0);
        userExam.getSbujectList().stream().forEach(subject -> {
        ...
        for (int i = 0; i < listresult.size(); i++) {
            ...
            row.createCell(1).setCellValue(listresult.get(i).getId());
            row.createCell(2).setCellValue(listresult.get(i).getName());
            //动态的列
            List<Subject> sbujectList = listresult.get(i).getSbujectList();
            for (int j = 0; j < sbujectList.size(); j++) {
                row.createCell(j + 3).setCellValue(sbujectList.get(j).getScore());
            }
